    ...
    public final void testCreateFrom() throws InvalidSettingsException, InvalidConfigEntryException {
        ...
        if (isCreationCompatible(ConfigEntries.xboolean)) {
            ...
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockBooleanConfig(toBoolean(testValue)), KEY).get());
        }
        // byte
        if (isCreationCompatible(ConfigEntries.xbyte)) {
            T testValue = getValueForTesting(IntType.INSTANCE);
            assertEquals(testValue, m_testInstance.createFrom(mocker.mockByteConfig(toByte(testValue)), KEY).get());
        }
        // short
        if (isCreationCompatible(ConfigEntries.xshort)) {
            T testValue = getValueForTesting(IntType.INSTANCE);
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockShortConfig(toShort(testValue)), KEY).get());
        }
        // int
        if (isCreationCompatible(ConfigEntries.xint)) {
            T testValue = getValueForTesting(IntType.INSTANCE);
            assertEquals(testValue, m_testInstance.createFrom(mocker.mockIntConfig(toInt(testValue)), KEY).get());
        }
        // long
        if (isCreationCompatible(ConfigEntries.xlong)) {
            T testValue = getValueForTesting(LongType.INSTANCE);
            assertEquals(testValue, m_testInstance.createFrom(mocker.mockLongConfig(toLong(testValue)), KEY).get());
        }
        // float
        if (isCreationCompatible(ConfigEntries.xfloat)) {
            T testValue = getValueForTesting(DoubleType.INSTANCE);
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockFloatConfig(toFloat(testValue)), KEY).get());
        }
        // double
        if (isCreationCompatible(ConfigEntries.xdouble)) {
            T testValue = getValueForTesting(DoubleType.INSTANCE);
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockDoubleConfig(toDouble(testValue)), KEY).get());
        }
        // char
        if (isCreationCompatible(ConfigEntries.xchar)) {
            T testValue = getValueForTesting(StringType.INSTANCE);
            assertEquals(testValue, m_testInstance.createFrom(mocker.mockCharConfig(toChar(testValue)), KEY).get());
        }
        // string
        if (isCreationCompatible(ConfigEntries.xstring)) {
            T testValue = getValueForTesting(StringType.INSTANCE);
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockStringConfig(testValue.toString()), KEY).get());
        }
        // transient string
        if (isCreationCompatible(ConfigEntries.xtransientstring)) {
            T testValue = getValueForTesting(StringType.INSTANCE);
            assertEquals(testValue,
                m_testInstance.createFrom(mocker.mockTransientStringConfig(testValue.toString()), KEY).get());
        }
    }
